...
class ReservaController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $vuelo = Vuelo::find($model->id_vuelo)->one();            
            Yii::$app->db->createCommand('UPDATE vuelo SET disponibles=disponibles -1 WHERE id_vuelo=:id_vuelo')
            ->bindValue(':id_vuelo', $model->id_vuelo)
            ->execute();
            if($model->save()){
